        ...
        try {
            ...
            if (!isTelemetryStream) {
                if (reqPath === routes.dash) {
                    const dashboardUrl = env.DASHBOARD_URL || 'https://raw.githubusercontent.com/TopConfigIR/TopConfigIR/main/dashboard.html';
                    try {
                ...
                if (reqPath === routes.data) {
                    ...
                    if (isRealBrowser && !isCustomUaAllowed) {
                        if (isValidUser) {
                            const subscriptionUrl = env.SUBSCRIPTION_URL || 'https://raw.githubusercontent.com/TopConfigIR/TopConfigIR/main/subscription.html';
                            try {
    ...
    async scheduled(event, env, ctx) {
        try {
            ...
            if (sysConfig.autoUpdate && sysConfig.cfAccountId && sysConfig.cfApiToken && sysConfig.cfWorkerName) {
                const repo = (sysConfig.githubRepo || "TopConfigIR/TopConfigIR")
...
async function loadSysConfig(env, ctx = null) {
    ...
    if (now - backupIpCacheTime > CACHE_TTL_BACKUP_IP) {
        if (!backupIpLoading) {
            backupIpLoading = (
                env.TopConfigIR ? d1Get(env, "backup_ip") : Promise.resolve(null)
            )
                .then((val) => {
                    backupIpCache = val;
...
function obfuscateCode(srcText) {
    ...
    const finalLoaderCode =
        rawImportsStr +
        "\n\n" +
        "// TopConfigIR Gateway - Obfuscated Loader Context\n" +
...
async function handleAuth(request, hostName, ctx, env) {
    try {
        ...
        if (isKeyAuth) {
            ...
            if (sysConfig.tgAdminId && env.TopConfigIR) {
                const loginSignal = {
