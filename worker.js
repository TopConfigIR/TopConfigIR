                ...
                ips.forEach((ip) => {
                    ...
                    _pips.forEach((selectedProxyIp) => {
                        ...
                        if (effectiveMode === "alpha" || effectiveMode === "both") {
                            ...
                            lines.push(`${vlessProto()}://${configUuid}@${ip}:${port}?${extBase}#${vName}`);
                        }
                        if (effectiveMode === "beta" || effectiveMode === "both") {
                            ...
                            lines.push(`${trojanProto()}://${p.id}@${ip}:${port}?${trojanExtBase}#${tName}`);
                        }
                        if (panelConfig.enableDirectConfigs && pips.length > 0 && selectedProxyIp === pips[0]) {
                            configIndex++;
                            let dvName = getConfigName("alpha", p.name, port, hName, ip, null, configIndex, ipName, true);
                            let dtName = getConfigName("beta", p.name, port, hName, ip, null, configIndex, ipName, true);
                            if (effectiveMode === "alpha" || effectiveMode === "both") {
                                let configUuid = generateConfigUuid(p.id, configIndex);
                                registerConfigEntry(configUuid, p.id, "");
                                lines.push(`${vlessProto()}://${configUuid}@${ip}:${port}?${extBase}#${dvName}`);
                            }
                            if (effectiveMode === "beta" || effectiveMode === "both") {
                                ...
                                lines.push(`${trojanProto()}://${p.id}@${ip}:${port}?${trojanExtBase2}#${dtName}`);
                            }
                        }
                        configIndex++;
                    });
                ...
                ips.forEach((ip) => {
                    ...
                    _pips.forEach((selectedProxyIp) => {
                        ...
                        if (panelConfig.enableDirectConfigs && pips.length > 0 && selectedProxyIp === pips[0]) {
                            ...
                            if (effectiveMode === "alpha" || effectiveMode === "both") {
                                let dvName = getUniqueName(getConfigName("alpha", p.name, port, hName, ip, null, dcIndex, ipName, true));
                            ...
                            if (effectiveMode === "beta" || effectiveMode === "both") {
                                let dtName = getUniqueName(getConfigName("beta", p.name, port, hName, ip, null, dcIndex, ipName, true));
            ...
            effectivePorts.forEach((port) => {
                ...
                ips.forEach((ip) => {
                    ...
                    _pips.forEach((selectedProxyIp) => {
                        ...
                        if (panelConfig.enableDirectConfigs && pips.length > 0 && selectedProxyIp === pips[0]) {
                            if (isVless) {
                                let tagStr = getUniqueName(getConfigName("alpha", p.name, port, hName, ip, null, configIndex, ipName, true));
                                dynamicTags.push(tagStr);
                            ...
                            if (isTrojan) {
                                let tagStr = getUniqueName(getConfigName("beta", p.name, port, hName, ip, null, configIndex, ipName, true));
                                dynamicTags.push(tagStr);
                                proxyGeoInfo.set(tagStr, getGeoInfo(ip));
                                let randomJunk = Array.from({ length: 11 }, () => "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"[Math.floor(Math.random() * 62)]).join("");
                                let payloadTr = { junk: randomJunk, protocol: "tr", mode: "proxyip", panelIPs: [], relayIdx: configIndex };
...
async function handleSubscription(request, url, env, ctx) {
    try {
        ...
        if (flag === "clash" || flag === "yaml" || flag === "meta" || flag === "stash" || flag === "clash-meta" || flag === "y") {
            ...
            if (ua.includes(clashProto()) || ua.includes("meta") || ua.includes("sta" + "sh") || ua.includes("verge") || ua.includes("mihomo") || ua.includes("cfw") || ua.includes("stash") || ua.includes("clash")) {
                isClashYaml = true;
